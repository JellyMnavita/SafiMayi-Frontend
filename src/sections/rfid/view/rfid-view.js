...
export function RFIDView() {
    ...
    const [formData, setFormData] = useState({}); // peut être une carte ou plusieurs
    ...
    const handleSave = async () => {
        try {
            ...
            if (mode === "single") {
                if (formData.id) {
                    // Edition
                    await axios.put(`https://safimayi-backend.onrender.com/api/rfid/update/${formData.id}/`, {
                    ...
                    // Création
                    await axios.post(`https://safimayi-backend.onrender.com/api/rfid/`, {
                        code_uid: formData.code_uid,
                        telephone: formData.telephone,
                    }, { headers: { Authorization: `Bearer ${token}` } });
                }
            ...
            if (mode === "multiple") {
                await axios.post(`https://safimayi-backend.onrender.com/api/rfid/`, formData.list || [], { headers: { Authorization: `Bearer ${token}` } });
            }
            if (mode === "auto") {
                await axios.post(`https://safimayi-backend.onrender.com/api/rfid/`, {
    ...
    const handleToggleActivation = async (code_uid) => {
        try {
            ...
            setRfids(prevRfids => prevRfids.map(rfid => rfid.code_uid === code_uid
                ...
                : rfid));
            await axios.post(`https://safimayi-backend.onrender.com/api/rfid/toggle/`, { uid: code_uid }, { headers: { Authorization: `Bearer ${token}` } });
            fetchRfids();
        }
        catch (error) {
    ...
    return (_jsxs(DashboardContent, { children: [_jsxs(Box, { sx: { mb: 5, display: "flex", alignItems: "center" }, children: [_jsx(Typography, { variant: "h4", sx: { flexGrow: 1 }, children: "Cartes RFID" }), _jsx(Button, { variant: "contained", color: "inherit", startIcon: _jsx(Iconify, { icon: "mingcute:add-line" }), onClick: () => {
                            ...
                            setOpenDialog(true);
                                ...
                                setSearchTel("");
                                setStatusFilter("");
                            }, children: "R\u00E9initialiser" })] }) }), _jsx(Grid, { container: true, spacing: 2, children: loading ? (_jsx(Typography, { children: _jsx(CircularProgress, {}) })) : rfids.length > 0 ? (rfids.map((rfid) => (_jsx(Grid, { size: { xs: 12, sm: 6, md: 4, lg: 3 }, children: _jsxs(Card, { sx: { p: 2, display: "flex", flexDirection: "column", gap: 1 }, children: [_jsxs(Box, { sx: { display: "flex", justifyContent: "space-between" }, children: [_jsx(Typography, { variant: "subtitle1", children: rfid.code_uid }), _jsx(IconButton, { onClick: (e) => handleMenuOpen(e, rfid), children: _jsx(Iconify, { icon: "eva:more-vertical-fill" }) })] }), _jsx(Typography, { variant: "body2", color: "text.secondary", children: rfid.telephone }), _jsx(Typography, { variant: "caption", color: "text.secondary", children: new Date(rfid.created_at).toLocaleDateString() }), _jsx(Box, { children: rfid.active ? (_jsx("span", { className: "bg-green-100 text-green-700 px-2 py-1 rounded-full text-xs font-medium", children: "Actif" })) : (_jsx("span", { className: "bg-red-100 text-red-700 px-2 py-1 rounded-full text-xs font-medium", children: "D\u00E9sactiv\u00E9" })) })] }) }, rfid.id)))) : (_jsx(Typography, { children: "Aucune carte RFID trouv\u00E9e" })) }), _jsx(Menu, { anchorEl: anchorEl, open: Boolean(anchorEl), onClose: handleMenuClose, children: _jsxs(MenuList, { children: [_jsx(MenuItemMui, { onClick: () => {
                                ...
                                handleMenuClose();
                            }, children: selectedRfid?.active ? "Désactiver" : "Activer" })] }) }), _jsxs(Dialog, { open: openDialog, onClose: () => setOpenDialog(false), fullWidth: true, maxWidth: "sm", children: [_jsx(DialogTitle, { children: formData.id ? "Éditer la carte RFID" : "Nouvelle carte RFID" }), _jsxs(DialogContent, { sx: { display: "flex", flexDirection: "column", gap: 2, mt: 1 }, children: [_jsx(Tabs, { value: mode, onChange: (e, v) => setMode(v), children: _jsxs(Tabs, { value: mode, onChange: (e, v) => setMode(v), children: [_jsx(Tab, { label: "Simple", value: "single" }), !formData.id && _jsx(Tab, { label: "Multiple", value: "multiple" }), !formData.id && _jsx(Tab, { label: "Auto", value: "auto" })] }) }), mode === "single" && (_jsxs(_Fragment, { children: [_jsx(TextField, { label: "Code UID", value: formData.code_uid || "", onChange: (e) => setFormData({ ...formData, code_uid: e.target.value }), fullWidth: true }), _jsx(TextField, { label: "T\u00E9l\u00E9phone", value: formData.telephone || "", onChange: (e) => setFormData({ ...formData, telephone: e.target.value }), fullWidth: true })] })), !formData.id &&
                                _jsxs(_Fragment, { children: [mode === "multiple" && (_jsxs(Box, { sx: { display: "flex", flexDirection: "column", gap: 2 }, children: [(formData.list || []).map((item, index) => (_jsxs(Box, { sx: { display: "flex", gap: 2, alignItems: "center" }, children: [_jsx(TextField, { label: "Code UID", value: item.code_uid || "", onChange: (e) => {
                                                    ...
                                                    }), children: "+ Ajouter une carte" })] })), mode === "auto" && (_jsx(_Fragment, { children: _jsx(TextField, { label: "Nombre de cartes", type: "number", value: formData.nombre || 1, onChange: (e) => setFormData({ ...formData, nombre: Number(e.target.value) }), fullWidth: true }) }))] })] }), _jsxs(DialogActions, { children: [_jsx(Button, { onClick: () => setOpenDialog(false), children: "Annuler" }), _jsx(Button, { variant: "contained", onClick: handleSave, children: "Enregistrer" })] })] })] }));
}
