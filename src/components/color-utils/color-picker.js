...
export function ColorPicker({ sx, value, onChange, slotProps, className, size = 36, options = [], limit = 'auto', variant = 'circular', ...other }) {
    ...
    return (_jsx(ColorPickerRoot, { limit: limit, className: mergeClasses([colorPickerClasses.root, className]), sx: [
            {
                '--item-size': `${size}px`,
                '--item-radius': (variant === 'circular' && '50%') ||
                    (variant === 'rounded' && 'calc(var(--item-size) / 6)') ||
                    '0px',
            },
            ...(Array.isArray(sx) ? sx : [sx]),
        ], ...other, children: options.map((color) => {
            const hasSelected = isSingleSelect ? value === color : value.includes(color);
            return (_jsx("li", { children: _jsx(ItemRoot, { "aria-label": color, onClick: () => handleSelect(color), className: colorPickerClasses.item.root, ...slotProps?.item, children: _jsx(ItemContainer, { color: color, hasSelected: hasSelected, className: colorPickerClasses.item.container, ...slotProps?.itemContainer, children: _jsx(ItemIcon, { color: color, hasSelected: hasSelected, icon: "eva:checkmark-fill", className: colorPickerClasses.item.icon, ...slotProps?.icon }) }) }) }, color));
        }) }));
}
// ----------------------------------------------------------------------
